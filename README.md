...
Clong the project and run `npm install` in the repo's root directory.
...
- LINK tokens (learn how to get them for each chain [here](https://documentation-private-git-ccip-documentation-chainlinklabs.vercel.app/resources/link-token-contracts?parent=ccip))
- CCIP-BnM Tokens for that chain using the `drip()` function (see [here](https://docs.chain.link/ccip/test-tokens#mint-test-tokens)) 
